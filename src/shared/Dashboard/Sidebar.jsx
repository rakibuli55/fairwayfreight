...
const Sidebar = () => {
    const menuItems = [
        ...
        {id:2, name:"Shipments", path:"/dashboard/shipment-history", icon:<BsBoxSeam />},
      ...
      <ul>
        {
            menuItems?.map((item) => ( 
                <li key={item?.id}>
                    <NavLink to={`${item?.path}`} className={`flex items-center gap-4 py-5 px-7 rounded-[16px] text-[18px] text-paragraph font-semibold mt-5 capitalize duration-200 ease-in-out hover:bg-primaryGreen hover:text-white ${item.path === pathname ? 'active-menu-link' : ''}`}> <span className="text-[20px]">
